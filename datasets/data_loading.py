...
from datasets.corruptions_datasets import create_cifarc_dataset, create_cifarc_lt_dataset, create_imagenetc_dataset
...
def get_transform(dataset_name: str, adaptation: str, preprocess: Union[transforms.Compose, None], use_clip: bool, n_views: int = 64):
    ...
    if use_clip:
        ...
        if dataset_name in ["cifar10", "cifar100", "cifar10_c", "cifar100_c", "cifar10_c_lt", "cifar100_c_lt"]:
        ...
        elif dataset_name in ["cifar10_c", "cifar100_c", "cifar10_c_lt", "cifar100_c_lt"]:
            transform = None
        elif dataset_name in ["imagenet_c", "ccc"]:
            # note that ImageNet-C and CCC are already resized and centre cropped (to size 224)
            transform = transforms.Compose([transforms.ToTensor()])
...
def get_test_loader(setting: str, adaptation: str, dataset_name: str, preprocess: Union[transforms.Compose, None],
    ...
    if domain_name == "none":
        ...
        elif dataset_name in ["cifar10_c_lt", "cifar100_c_lt"]:
            # Long-tail controls are env-driven so experiments can stress the
            # dataset without changing the main dataset names.
            _if = float(os.environ.get("CAMA_LT_IMBALANCE_FACTOR", "100"))
            _ls = int(os.environ.get("CAMA_LT_SEED", "0"))
            _order = os.environ.get("CAMA_LT_CLASS_ORDER", "index")
            _order_seed = int(os.environ.get("CAMA_LT_ORDER_SEED", "0"))
            test_dataset = create_cifarc_lt_dataset(dataset_name=dataset_name,
                                                     severity=severity,
                                                     data_dir=data_dir,
                                                     corruption=domain_name,
                                                     corruptions_seq=domain_names_all,
...
def get_source_loader(dataset_name: str, adaptation: str, preprocess: Union[transforms.Compose, None],
    ...
    if dataset_name in ["cifar10", "cifar10_c", "cifar10_c_lt"]:
        source_dataset = torchvision.datasets.CIFAR10(root=data_root_dir,
                                                      train=train_split,
                                                      download=True,
                                                      transform=transform)
    elif dataset_name in ["cifar100", "cifar100_c", "cifar100_c_lt"]:
