...
# ----------------------------- Trusted TTA options ----------------------- #
_C.TRUSTED_TTA = CfgNode()

# Margin threshold for Condition 1 gate (sample with margin <= tau_margin is excluded)
_C.TRUSTED_TTA.TAU_MARGIN = 0.5

# EMA decay for class prototype accumulation (higher = more stable, slower adaptation)
_C.TRUSTED_TTA.EMA_ALPHA = 0.9

# Loss weights for I2T alignment and InterMean separation terms
_C.TRUSTED_TTA.LAMBDA_I2T = 1.0
_C.TRUSTED_TTA.LAMBDA_INTER = 1.0

# Option 2 (multiview): number of augmented views for majority-vote filter
_C.TRUSTED_TTA.N_AUG = 5

# Option 3 (knn_cache): k for kNN classifier and feature cache size
_C.TRUSTED_TTA.KNN_K = 10
_C.TRUSTED_TTA.CACHE_SIZE = 2000

# -------------------- Projected Evidence-Gated BATCLIP options ----------- #
_C.PROJECTED_BATCLIP = CfgNode()

# Component 2: evidence gate threshold for s_max.
# Set to "median" for dynamic per-batch median, or a float (e.g. 0.25).
_C.PROJECTED_BATCLIP.TAU_GATE = "median"

# Component 3: maximum recirculation steps per batch (1 = no recirculation).
_C.PROJECTED_BATCLIP.MAX_RECIRC_STEPS = 1

# Component 3: d_eff_parallel improvement below this threshold counts as plateau.
_C.PROJECTED_BATCLIP.DEFF_EPS = 0.01

# Component 3: number of consecutive plateau steps before early stopping.
_C.PROJECTED_BATCLIP.PLATEAU_PATIENCE = 3

# ----------------------------- Geometric TTA options --------------------- #
_C.GEOMETRIC_TTA = CfgNode()

# Sinkhorn entropy regularization (controls sharpness of OT plan)
_C.GEOMETRIC_TTA.EPSILON = 0.1

# Decaying potential field decay coefficient
_C.GEOMETRIC_TTA.ALPHA = 10.0

# Weight of L_inter repulsion loss
_C.GEOMETRIC_TTA.LAMBDA = 1.0

# Number of Sinkhorn iterations
_C.GEOMETRIC_TTA.N_SINKHORN = 20

# Number of Weiszfeld iterations for Fréchet mean approximation
_C.GEOMETRIC_TTA.N_WEISZFELD = 2

# -------------------------- SoftLogitTTA options ------------------------- #
_C.SOFT_LOGIT_TTA = CfgNode()

# EMA decay for prior histogram (higher = slower update, more stable)
_C.SOFT_LOGIT_TTA.BETA_HIST = 0.9

# Prior correction strength λ_adj
_C.SOFT_LOGIT_TTA.LAMBDA_ADJ = 1.0

# Clamp magnitude on Δ_c (prevents over-correction)
_C.SOFT_LOGIT_TTA.CLIP_M = 3.0

# Sigmoid sharpness α_s for MAD-scaled soft weights
_C.SOFT_LOGIT_TTA.ALPHA_S = 2.0

# Cosine margin for softplus repulsion (margin_pot)
_C.SOFT_LOGIT_TTA.MARGIN_POT = 0.3

# Softplus scale γ for repulsion
_C.SOFT_LOGIT_TTA.GAMMA_POT = 10.0

# Loss component weights
_C.SOFT_LOGIT_TTA.W_I2T = 1.0    # soft I2T alignment weight
_C.SOFT_LOGIT_TTA.W_POT = 1.0    # L_pot weight
_C.SOFT_LOGIT_TTA.W_UNI = 0.5    # L_uni weight
...
def complete_data_dir_path(data_root_dir: str, dataset_name: str):
    ...
    mapping = {"imagenet": "imagenet2012",
               ...
               "cifar10_c_lt": "",  # long-tail CIFAR-10-C view, data path shared with CIFAR-10-C
               "cifar100": "",      # do not change
               "cifar100_c": "",    # do not change
               "cifar100_c_lt": "", # long-tail CIFAR-100-C view, data path shared with CIFAR-100-C
